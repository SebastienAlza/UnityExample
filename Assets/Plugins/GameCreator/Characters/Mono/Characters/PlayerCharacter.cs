    ...
    public class PlayerCharacter : Character
    {
        ...
        protected virtual void Update()
        {
            ...
            switch (this.inputType)
            {
                ...
                case INPUT_TYPE.SideScrollXZ: this.UpdateInputSideScrollXZ(); break;
        ...
        protected Vector3 lastValidDirection = Vector3.forward; // Ajoutez cette variable
        ...
        protected virtual void UpdateInputSideScroll(Vector3 axis)
        {
            ...
            else
            {
                ...
                {
                    targetDirection = axis * Input.GetAxis(AXIS_H);
                    // Supprimez la rotation sur l'axe vertical (Y)
            ...
            // Stockez la dernière direction valide
            lastValidDirection = targetDirection;
        }

        protected virtual void UpdateInputSideScrollXZ()
        {
            if (!this.IsControllable()) return;

            Vector3 targetDirection = Vector3.zero;

            // Gérer les entrées des plateformes mobiles et non mobiles
            if (Application.isMobilePlatform ||
                TouchStickManager.FORCE_USAGE ||
                this.forceDisplayTouchstick)
            {
            ...
            // Appliquer le mouvement sans changer la rotation du sprite
            Vector3 moveDirection = targetDirection.normalized;
            targetDirection.y = 0.0f;
            this.characterLocomotion.SetDirectionalDirection(moveDirection);

            // Maintenir la rotation originale du sprite constante
            targetDirection.y = 0.0f;
        }
